...
mod tests {
    ...
    fn test_price_to_bps() {
        ...
        assert_eq!(price_to_bps(0.5), 5000);
    ...
    fn test_size_to_micro() {
        assert_eq!(size_to_micro(100.0), 100_000_000);
    ...
    fn test_get_order_amounts_buy() {
        // BUY 100 contracts at 0.65 price
        ...
        assert_eq!(side, 0); // BUY
        // taker = size = 100_000_000 (what we receive)
        assert_eq!(taker, 100_000_000);
        // maker = size * price / 10000 = 100_000_000 * 6500 / 10000 = 65_000_000 (what we pay)
        assert_eq!(maker, 65_000_000);
    }

    #[test]
    fn test_get_order_amounts_sell() {
        // SELL 100 contracts at 0.65 price
        let size_micro = size_to_micro(100.0); // 100_000_000
        let price_bps = price_to_bps(0.65);     // 6500
        let (side, maker, taker) = get_order_amounts_sell(size_micro, price_bps);

        assert_eq!(side, 1); // SELL
        // maker = size = 100_000_000 (what we give)
        assert_eq!(maker, 100_000_000);
        // taker = size * price / 10000 = 100_000_000 * 6500 / 10000 = 65_000_000 (what we receive)
        assert_eq!(taker, 65_000_000);
    }

    #[test]
    fn test_price_valid() {
        assert!(!price_valid(0));    // Too low
        assert!(!price_valid(99));   // Just below min
        assert!(price_valid(100));   // Min valid (0.01)
        assert!(price_valid(5000));  // 0.50
        assert!(price_valid(9900));  // Max valid (0.99)
        assert!(!price_valid(9901)); // Just above max
        assert!(!price_valid(10000)); // Too high
    }
    ...
    fn test_exchange_addresses() {
        ...
        // Testnet neg_risk
        assert_eq!(
            get_exchange_address(80002, true).unwrap(),
            "0xd91E80cF2E7be2e162c6513ceD06f1dD0dA35296"
        );
        // Testnet non-neg_risk
        assert_eq!(
            get_exchange_address(80002, false).unwrap(),
